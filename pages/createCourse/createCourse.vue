/** * 创建课程 * 表单获取用户输入的：课程编号、课程名、课时数、周课时数、课程描述，自动填充创建时间、创建人。 */

<template>
	<view class="window">
		<form @submit="formSubmit">
			<view class="uploadCover"><robby-image-upload v-model="imageData" :server-url="serverUrl" fileKeyName="file"></robby-image-upload></view>
			<view class="uni-form-item uni-column">
				<view class="title">课程代码</view>
				<view><input class="uni-input formitem" :class="{ formerror: courseCodeError }" type="number" name="courseCode" placeholder="(必填)*" confirm-type="next" /></view>
			</view>
			<view class="uni-form-item uni-column">
				<view class="title">课程名称</view>
				<view><input class="uni-input formitem" :class="{ formerror: courseNameError }" type="text" name="courseName" placeholder="(必填)*" confirm-type="next" /></view>
			</view>
			<view class="uni-form-item uni-column">
				<view class="title">课时数</view>
				<view>
					<input class="uni-input formitem" :class="{ formerror: courseCountError }" type="number" name="courseCount" placeholder="(必填)*" confirm-type="next" />
				</view>
			</view>
			<view class="uni-form-item uni-column">
				<view class="title">周课时</view>
				<view>
					<input class="uni-input formitem" :class="{ formerror: courseWeekCountError }" type="number" name="courseWeekCount" placeholder="(必填)*" confirm-type="next" />
				</view>
			</view>
			<view class="title">课程描述:</view>
			<view class=" uni-textarea"><textarea class="uni-input formitem uni-form-item uni-column" name="courseMemo" placeholder="课程描述" confirm-type="done" /></view>
			<view class="submit"><button type="primary" form-type="submit">提交课程信息</button></view>
		</form>
	</view>
</template>
...
export default {
	...
	methods: {
		formSubmit(e) {
			//从form表单中获取结果 e
			console.log('course formSubmit', e.detail.value); //控制台中打印结果e的详细信息
			const formData = e.detail.value; //将结果e中detail目录下的value中的信息录入formData中
			formData.courseCover = this.imageData.join(','); //向formData中添加新字段courseCover并赋值，赋值对象是imageData，赋值方法为join(拼接)
			const validResult = this.formValid(formData); //将整理好的formData信息提交给formValid方法判断是否合法，并将判断结果赋值给validResult
			if (!validResult) {
				//判断结果validResylt，若为false则弹出uni组件中的showToast弹框提示"请填写完整的课程信息"并return空结束,为true则执行下面的代码
				uni.showToast({
					icon: 'none',
					title: '请填写完整的课程信息'
				});
			...
			this.courseService
				.createCourse(formData) //调用courseService中的createCourse方法，将提取出的有效信息formData传给创建课程的接口
				.then(result => {
