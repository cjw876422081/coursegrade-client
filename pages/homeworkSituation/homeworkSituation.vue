	...
	<view class="main-content">
		<mescroll-uni :down="downOption" @down="downCallback" :up="upOption" @up="upperCallback">
			...
			<view class="studenthomework" v-if="flagII">
				...
				<uni-card v-for="(student,i) in allStudentHomework" :key="i" v-if="student.submitMemo&&student.grade" style="margin-bottom:10rpx;background: #f0ffff;"
				 :title="student.student" :extra="'得分:'+student.grade">
			...
			<view>
				...
				<view v-if="flagI">
					...
					<uni-card v-for="(student,i) in allStudentHomework" :key="i" v-if="!student.grade && student.submitMemo" style="margin-bottom:10rpx;background: #f0ffff;"
					 :title="student.student" :extra="'得分:'+student.grade">
						<label>作业内容:</label><br><br>
						<text style="word-wrap: break-word;word-break: break-all;">{{student.submitMemo}}</text><br><br>
						<label>提交时间:{{student.submitTime}}</label><br>
			...
			<view>
				...
				<view v-if="flag">
					...
					<uni-list v-for="(student,i) in allStudentHomework" :key="i" v-if="!student.submitMemo">
						<uni-list-item :title="student.student" show-arrow="false" show-extra-icon="true"></uni-list-item>
					</uni-list>
				</view>
			</view>
		</mescroll-uni>
					...
					if (this.pageIndex == 0) {
						...
						for (let i = 0; i < this.allStudentHomework.length; i++) {
							...
							if (this.allStudentHomework[i].submitMemo == null ||
								this.allStudentHomework[i].submitMemo == "") {} else {
								count++;
							}
						}
						//计算提交并评分人数
