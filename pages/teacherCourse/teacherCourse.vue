		...
		<mescroll-uni :down="downOption" @down="downCallback" :up="upOption" @up="upperCallback">
			<view v-for="(course, i) in courses" :key="i" style="margin-bottom:10rpx;" @click="courseClick(course)">
				<uni-swipe-action :options="course.delOptions" @click="delClick" data-course="course">
					<uni-card :title="course.courseName" thumbnail="/static/logocolor.png" :extra="course.courseCode">
						<text>{{ course.courseMemo }}</text>
					</uni-card>
				</uni-swipe-action>
			</view>
		</mescroll-uni>
		<uni-fab horizontal="right" :content="fabButtonContent" @trigger="addClick"></uni-fab>
	</view>
</template>
<style>
.main-content {
...
export default {
	...
	methods: {
		...
		courseClick(course) {
			...
			uni.navigateTo({
				url: '../coursePlan/coursePlan-teacher?courseId=' + course.id
			});
		},
		delClick(e) {
